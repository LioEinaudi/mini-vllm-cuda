...
def test_rmsnorm_matches_torch_ref(hidden_size, num_tokens, dtype, rtol, atol):
    torch.manual_seed(0)
    ...
    print(
        f"RMSNorm dtype={dtype} num_tokens={num_tokens} hidden_size={hidden_size} "
        f"max_error={max_error:.6g} mean_error={mean_error:.6g} allclose={is_close}"
    )
    assert is_close, (
        f"RMSNorm mismatch: dtype={dtype}, num_tokens={num_tokens}, "
